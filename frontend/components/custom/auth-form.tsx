...
type fieldSetterType = (param: string) => void;

const AuthForm = ({ caption, submitCaption, fields, setters }: {
  caption: string,
  submitCaption: string,
  fields: Array<string>,
  setters: Array<fieldSetterType>
}) => {

  const [email, password, confirmPassword] = fields;
  const [setEmail, setPassword, setConfirmPassword] = setters;
    ...
    <div className=' lg:w-[60%] h-fit flex flex-row justify-center mx-auto my-20 p-4 border-2 border-gray-50 rounded-2xl shadow-lg shadow-gray-200'>
      ...
      <div className='ml-2 self-start'>
        ...
        <form action="" className='my-6'>
          ...
          <div className='my-3'>
            <Button className='w-36 bg-brown-100' type='submit'>{submitCaption}</Button>
          </div>
        </form>
      </div>
    </div >
